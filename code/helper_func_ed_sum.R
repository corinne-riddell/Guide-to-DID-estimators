...
fit_event_manual_sum <- function(outcome_var, date_var,
...
fit_event_jack_sum <- function(outcome_var, date_var, unit_var,
                           policy_var, data, max_time_to = NULL) {
  ...
  map_df(units,
         function(i) {
           ...
           fit_event_manual_sum(outcome_var, date_var,
                            policy_var, data[data[[unit_var]] != i,],
                            max_time_to) %>%
             mutate(dropped = i)
         }) -> jack_ests
  # compute standard errors and join in with results
  full_res <- fit_event_manual_sum(outcome_var, date_var, policy_var,
                               data, max_time_to)
  jack_ests %>%
