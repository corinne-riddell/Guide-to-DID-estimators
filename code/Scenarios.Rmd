...
* Paper about the Goodman-Bacon Decomposition: Goodman-Bacon A. Difference-in-differences with variation in treatment timing. *J Econom*. 2021 Jun;S0304407621001445.  

* Paper about the Group-Time ATT Estimator: Callaway B, Sant’Anna PHC. Difference-in-Differences with multiple time periods. *J Econom*. 2020 Dec;S0304407620303948. 

* Paper about the Cohort ATT Estimator: Sun L, Abraham S. Estimating dynamic treatment effects in event studies with heterogeneous treatment effects. *J Econom*. 2021 Dec;225(2):175–99. 
...
`r colorize("It is saying that there is not other change in the treated group but not the control group at the exact moment the treatment is introduced -- like if there was two policies introduced at that moment but you are only considering one of them -- then the causal effect is due to both but you attribute it to just one thing", "red")`
...
the policy change increases by one in each successive period. This table 
summarizes the causal effect over time since treatment:
...
aggregates across all post-treatment time into one number. In this case, we can 
...
$(3 + 4 + ... + 18)/16 = 10.5$. Note that this equals the average dynamic effect
that we were aiming to estimate.
...
1. **Time-specific dynamic effects**
...
their average at each time point in the last column. When estimating the effects 
dynamically, these are the time-since-treatment-specific parameters we aim to estimate.

2. **The average dynamic effect**: You may prefer a summary estimate that aggregates
across all post-treatment time into one number. In this case, we can take the
average of the causal effects across all the time points:
...
This quantity is much lower than the average dynamic effect of 16.31 we aim to 
estimate. Why is the TWFE estimate so much lower? In the previous scenario we 
...
contrast in this setting did not satisfy the parallel trends assumption, but the other three
contrasts did. How much weight did the TWFE estimator put on each of the contrasts?
Let's consider the Goodman Bacon decomposition to see how 
...
The Group-Time estimate is `r s8_cs_ag$overall.att %>% round(2)` with 95% CI `r lb %>% round(2)` 
to `r ub %>% round(2)`.
...
Group-Time estimates plot, showing no discernible trends in the pre-exposure
period, and an increasing effect of treatment in the post-exposure period.
