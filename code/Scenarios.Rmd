...
1. Goin DE, Riddell CA (2022). Comparing two-way fixed effects and new estimators for difference-in-differences: A simulation study and empirical example. Submitted. ADD LINK.

2. Riddell CA, Goin DE (2022). Guide for comparing estimators of policy change effects on health. Submitted. ADD LINK.
...
# Scenario 7 (4 states, staggered timing, dynamic and heterogeneous treatment effects)
...
2. **The average effect**: You may prefer a summary estimate that 
aggregates across all post-treatment time into one number. In scenario 6 we 
outlined two ways to summarize the estimate. Here we add a third way that can 
be used when the effect is heterogeneous by state.
...
2.ii) **Weighted average of the dynamic effects**: Take a weighted average of the 
causal effects, where the weights correspond to the number of units treated at
each time point:
...
2.iii) **Simple average of the average heterogeneous effects**: Take a simple 
average of the heterogeneous treatment effects:
...
is equal to the weighted average of the dynamic effects calculated in Section 9.2.
...
equal to the simple average of the heterogeneous effects calulated in Section 9.2.
...
time. This is equal to (2 + 3.5 + 5 + 6.5 + ... + 33)/16=16.31, which we also 
