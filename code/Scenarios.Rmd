...
1. Goin DE, Riddell CA (2022). Comparing two-way fixed effects and new estimators for difference-in-differences: A simulation study and empirical example. *Epidemiology*. 2023;34(4):535-543. doi: 10.1097/EDE.0000000000001611. [Link.](https://pmc.ncbi.nlm.nih.gov/articles/PMC10771800/)

2. Riddell CA, Goin DE (2022). Guide for comparing estimators of policy change effects on health [Letter to the Editor]. *Epidemiology*. 2023;34(3):e21-22. doi:10.1097/EDE.0000000000001586. [Link.](https://pmc.ncbi.nlm.nih.gov/articles/PMC10771122/)
...
## Follow along from the code file

Once the RStudio project is launched, use the "Files" tab to open the code folder
and select the "Scenarios.Rmd" file. Navigate to the section that loads the 
R packages and functions (line 121) to follow along with all the code displayed
on this website!
...
# Require the dev version of patchwork to avoid errors:
...
```{r}
source(here("code", "helper_func_ed.R"))
source(here("code", "helper_func_ed_sum.R"))
source(here("code", "./helper_func_ed_sum_hte.R"))
source(here("code", "./helper_func_ed_sum_C.R"))
```
...
ggsave(filename = here("images/scen1.png"), 
       width = 5, height = 3.5, device = "png", dpi = 600)
```
...
* Causal effect estimate of policy = 3-1 = 2. Equivalently, 
* State 1 difference of 5
* State 2 difference of 3
* Causal effect estimate of policy = 5-3 = 2. 

Three standard assumptions are typically stated in DID papers and required for the 
DID estimate to equal the causal effect of interest:

1) **Parallel trends:** The treated and untreated groups have parallel trends in
the outcome in the pre-treatment period. Had the treated group not been treated,
the trends for the treated state would continue to be the same as the 
never-treated state during the post-treatment time period. 
...
these assumptions in [their paper](https://pubmed.ncbi.nlm.nih.gov/34001754/), and we 
include the assumptions here for reference:

4) **Consistency:** There are no multiple versions of treatment that are unknown
to the investigator. For example, if states introduced a ban on texting while 
driving and if these bans were associated with different penalties, then the policy
...
## Visualization of Scenarios 2a and 2b
...
* Causal effect estimate of policy = 5-3 for both states for an average treatment effect on the treated (ATT) of 2
...
* Causal effect estimate of policy = 2 for state 2 vs 1 and 3 for state 3 vs 1 for an ATT of 2.5
...
states, then the estimated effect is the average of the ATTs across the 
states.
...
## Visualization of Scenario 3a and 3b
...
ggsave(filename = here("images/scen3.png"), 
       width = 10, height = 3.5, device = "png", dpi = 600)
```
...
time is important for understanding the real-world effects of the policy. 

In this scenario we first estimate the average treatment effect using a 
...
s4_mean_info <- s4 %>% group_by(state, post) %>%
  summarise(mean_y = mean(outcome), .groups = "drop") %>% 
  pivot_wider(id_cols = state, names_from = post, names_prefix = "post_", values_from = mean_y) %>%
  mutate(diff = post_TRUE - post_FALSE)
```
...
the time and state fixed effects are specified by the `time_var`and `id_var` 
arguments.
...
This small table illustrates that 100% of the weight is on comparisons between
...
```{r, warning=F}
s4_cs <- att_gt(yname = "outcome", #outcome variable name
             tname = "time", #time variable name
             idname = "state_n", #id variable name
             gname = "time_first_treat", #time of first treatment variable name (see instructions above)
             data = s4, 
...
heed warnings and understand why they occur.
...
ggsave(filename = here("images/scen6.png"), 
       width = 6, height = 3.5, device = "png", dpi = 600)
```
...
ggsave(filename = here("images/scen6-contrasts.png"), 
       width = 10, height = 7, device = "png", dpi = 600)

ggsave(s6_comp1, filename = here("images/scen6-contrast1.png"), 
       width = 5, height = 3.5, device = "png", dpi = 600)
```
...
ggsave(filename = here("images/scen8.png"), 
       width = 6, height = 3.5, device = "png", dpi = 600)
```
...
ggsave(filename = here("images/scen8-contrasts.png"), 
       width = 10, height = 7, device = "png", dpi = 600)
```
...
```{r include = F}
dat = data.frame(t = seq(-10, 15), att_t = s8_cs_ag3$att.egt)

ggplot(dat, aes(x = t, y = att_t)) + 
  geom_point(aes(col = t >= 0)) + 
  theme_bw() + labs(x = "Time", y = "ATT") +
  guides(col = "none")

ggsave(filename = here("images/scen8_event-study-no-se.png"), 
       width = 6, height = 3.5, device = "png", dpi = 600)
```
