...
`r colorize("TODO1: check with Dana that I am using the correct fit_event_jack functions across the different scenarios", "purple")`
...
analysis can be applied. We assume readers know what a DID design is and have some background knowledge in policy analysis. Reviewing the manuscript by Goin and 
Riddell and its references is a great place to start if this guide leaves you 
...
open the Guide-to-DID-estimators.Rproj file by double clicking it or by using the
...
found in Ben-Michael's linked repository. The other helper functions are ones
...
`staggered` version 1.1 and `did` version 2.2.0.901. If your results differ or you 
...
these in their paper, and we include them here for reference:
...
**Takeaway:** When there is one treated group and one control group, with 
...
`r colorize("there is an error when try to use nevertreated: Error in pre_process_did... never treated group is too small, try setting control_group to notyettreated", "purple")`
...
produce one overall estimate using the `fit_event_jack_sum()` and 
`fit_event_jack_sum_hte()` functions. Use `fit_event_jack_sum()` when there is 
only one adoption cohort, as shown here.
...
s6_bacon
```
We can see that the estimates of the treatment effect for each contrast are as we
calculated them earlier. Note that `r round(s6_bacon$weight[3]*100)`% of the 
weight of the TWFE estimate is on Contrast 4 -- the one making the improper 
comparison between an earlier treated and a later treated state.
...
This estimate considered only the contrasts with a never-treated state (i.e., 
Comparisons 1 and 2 in the figure above) and combines them into a weighted 
average, where the weights correspond 
to each adoption cohort's time spent in the post-treatment period. For contrast 
1, there are 16 time periods post treatment and for contrast 2, there are 9 
periods post-treatment. Thus the weighted average is: $10.5\times(16/25) + 7\times(9/25)=$ `r 10.5*(16/25) + 7*(9/25)`.
...
units in each adoption cohort. Thus, the researcher may want to estimate the ATT
...
Going back to the dynamic effect estimation, it is very helpful to plot the 
estimates over time using the `ggdid()` function:
...
Overall ATT from the Group-Time estimator when `type = "dynamic"`.
...
`type = "dynamic"` and the Target Trial approach.
...
researcher is identifying the parameter of interest -- are you interested in 
estimating a dynamic effect, or does a parameter that summarizes over treatment
time make sense? 

`r colorize("I added the recommendation below We can consider moving it to the end", "purple")`
...
time. This is equal to (2 + 3.5 + 5 + 6.5 + ... + 33)/16=16.31)
...
`r colorize("CR write up this section", "red")`
...
gives estimates that are systematically lower than those produced by the Group-Time 
estimator and the Target Trial estimator. 

**Takeway:** When the treatment effect is staggered, dynamic, and heterogeneous, 
...
case, it is best to use one of the newer estimators. 
...
if it's unable to estimate standard errors. To use the `staggered_sa` function, 
...
For this example, the Group-Time estimates and Cohort ATT estimates are quite
