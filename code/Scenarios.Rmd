...
`r colorize("Hi Dana, thanks for reading this over. If you leave comments, can you use this colorize() function (see the Rmd code used to show this sentence in blue. This will help ensure I can easily find your comments.", "blue")`

`r colorize("I got the Ben-Micheal approach working for Scenario 8 (by event time) and think the functions work for Scenario 4 and 5. But I can't seem to get the _sum functions to work for Scen 6 to 7. Let me know if you have an idea what is going wrong/can get it to work.", "blue")`
...
Submit any bug reports to: c.riddell [AT] berkeley [DOT] edu or open an issue on [Github](https://github.com/corinne-riddell/Guide-to-DID-estimators).

## Overview

In this companion guide, we illustrate scenarios in which difference in differences (DID)
analysis can be applied. Importantly, this is a **companion** guide -- meaning, it 
does not stand alone. We assume readers know what a DID design is and has some 
background knowledge outlined in the 
corresponding manuscript and previous literature. Reviewing the manuscript and its
references is a great place to start if this guide leaves you with more questions
than answers.

`r colorize("I would consider changing this section, maybe streamlining to the following:

In this guide, we illustrate scenarios in which difference in differences (DID)
analysis can be applied. We assume readers know what a DID design is and have some 
background knowledge in policy analysis. Reviewing the manuscript and its
references is a great place to start if this guide leaves you with more questions
than answers.", "blue")`


We start with the simplest of difference in differences scenarios and slowly amp
...
### Download the data and code used in this analysis
...
By default, this creates a new folder on your desktop (though you can specify a 
directory using the `destdir` argument in the `use_course()` function.)

The files will open in RStudio when you run the code. In the future, you can 
option the Guide-to-DID-estimators.Rproj file by double clicking it or use the
Select "File" > "Open Project" within RStudio and choose the Guide-to-DID-estimators
project.

`r colorize("Did you mean "open" rather than "option" in the above paragraph?", "blue")`


### Load the R packages and functions used in this analysis
...
The target trial estimator was implemented using code revised from the creator's
[GitHub repository](https://github.com/ebenmichael/policy-trial-emulation). We
use `source()` to read in the functions here. This code will only 
work for you if you downloaded the files using the `use_course()` function in 
the previous step.:
...
found in Ben-Micheal's linked repository. The other helper functions are ones
Dana Goin made to aggregate the estimates under different settings.

`r colorize("Ben-Micheal should be Ben-Michael", "blue")`
...
to reproduce the results. Here are the packages we are using in the analyses,
if your results differ, it may be because you are using newer versions of the 
packages.
...
DID estimate `r colorize("[in the simple case?]", "red")` to estimate the causal 
effect of interest:
...
policy change.
3) `r colorize("[NAME FOR THIS ONE:]", "red")` The policy is the only thing that changes at the time of
treatment that affects the outcome.

`r colorize("How is 3) different from 1)? Seems like if something other than the policy changing affected the outcome the trends wouldn't be parallel", "blue")`
...
`r colorize("Ben-Micheal should be Ben-Michael. I also think you should change this to "... list these in their paper, and we include them here for reference", "blue")`
...
consistency assumption because the different penalties may have varyed effects 
on the outcome.

`r colorize("I think we need to be more precise about how this differs from a heterogeneous effect. What about: 
There are no multiple versions of treatment that are unknown to the investigator. For 
example, if states introduced a texting ban while driving and these bans were 
associated with different penalties, then the policy change would not meet the
consistency assumption because the different penalties may have varied effects 
on the outcome and the investigator is analyzing these policies as though they were the same. However, if the investigator knows that the penalties differed and estimated 
the effects accounting for this heterogeneity, then the consistency assumption would not be violated.", "blue")`

5) **No interference:** Outcomes of individuals in one state do not affect 
outcomes of individuals in other states. `r colorize("Actually, this should be policies that affect individuals in one state should not affect the outcomes of individuals in other states", "blue")`
...
causal assumption is the same as the parallel trends assumption. `r colorize("[DANA DO YOU AGREE?]", "red")`
`r colorize("Yes, although I think we should also say that the potential outcomes are independent of the treatment received.", "blue")`
7) **Positivity:** Each state has a non-zero probability of being treated. 
`r colorize("DISCUSS: See Ben-Micheal paper - they say that positivity is not a required assumption for standard DID]", "red")`

`r colorize("Maybe we can also add: This is not required for DID estimators which use a linear model, which instead relies on the parametric model to extrapolate if necessary.", "blue")`

8) **Correct model specification**: `r colorize("SOMETHING ABOUT LINEARITY? ALSO RELATED TO THE PARALLEL TRENDS ASSUMPTION", "red")`

`r colorize("Do you think we need this if we have the rest of the assumptions? I'm trying to think what this covers that isn't already listed and can't think of anything", "blue")`
...
change. 
`r colorize("I would add: The indicator should be 1 for when the treated states are in the post-treatment period, and 0 otherwise", "blue")` The effect estimate is the regression coefficient on the policy variable.
...
`r colorize("Do you have a sense why the state FEs in Scen 2b are a bit off? The FE for state 2 should be 1 and for state 3 should be 2.", "red")`

`r colorize("I think this is because neither state actually has a treatment effect of 2.5. So I'm guessing it borrows from the FE for each state to make the averaging work.", "blue")`
...
time, suggesting that the policy change takes time to "kick-in". 

`r colorize("Or, there is a positive feedback loop of some kind. If it take a while to kick in then it would level off at some point.", "blue")`
...
The coefficient of the policy term `r colorize("for scenario 3A.", "blue")` is `r s3a_mod$coefficients["policy1"]`
...
The coefficient of the policy term `r colorize("for scenario 3B.", "blue")`is `r s3b_mod$coefficients["policy1"]`
showing that the regression model estimate equaled the causal effect.
...
Note that we could have modeled `time_since_policy` linearly `r colorize("by this do you mean you could just include a time_since_policy numeric variable?", "blue")`, however including 
`time_since_policy` as a factor variable allows the estimation of effects to be
non-linear over time.`r colorize("I think I would say instead, ".. allows the effects to change non-linearly over time"", "blue")`

`r colorize("May want to blod the "Takeaway" to be consistent", "blue")`
Takeaway: When there are two treatment groups and multiple pre- and post- time points, you can calculate the DID estimate using TWFE regression when the effect is constant 
or dynamic. `r colorize("Consider saying when there is one treatment and one control group? Otherwise two treatment groups sounds like you can have two treated groups, which then introduces possibility for staggered adoption.", "blue")`

Using a "time since treatment" indicator will estimate effects 
separately by time since the policy change, allowing you to see how the effect 
has changed over time.

## Scenario 4 (4 states, hetergenous treatment effects)
...
`r colorize("Do we want to say Overall ATT = mean(2,1,3) = 2? I feel like that might be helpful to show how the individual results are summarized into one overall estimate.", "blue")`
...
term. While all states are treated at the same time in this scenario, we can use
the Goodman-Bacon decomposition function `bacon()` to partition the effect 
estimate into weighted component parts. 

`r colorize("I don't really get why you are introducing Goodman Bacon and the other estimators here since there is no staggered adoption. I feel like they should come later. Or if you want to introduce them here, I would add more to their introduction that basically says, you don't need to use these estimators in this situation because the standard DID approach still works, but we show how they work to show you get the same answer with all of them.", "blue")`
...
`r colorize("Is there a reason why you used "notyettreated" rather than "nevertreated" for the control group here?, "blue")`
...
`r colorize("change Ben-Micheal to Ben-Michael", "blue")`
We can also calculate the estimate using the target trial estimator introduced 
by Ben-Micheal, Feller, and Stuart. We employ slightly edited versions of the 
code that these authors provide with their published article. The code 
calculates estimates of the effect for every 
combination of adoption cohort and event time, as we as an estimate aggregated 
across adoption cohorts for each event time. We built upon this code to further
aggregate across event times to produce one estimate across all post-treatment 
times using the `fit_event_jack_sum()` and `fit_event_jack_sum_hte()` functions.
Here we use `fit_event_jack_sum()` because there is only one adoption cohort. 

`r colorize("Change this: "The code 
calculates estimates of the effect for every 
combination of adoption cohort and event time, as we as an estimate aggregated 
across adoption cohorts for each event time. We built upon this code to further
aggregate across event times to produce one estimate across all post-treatment 
...
to this instead: "The code 
calculates estimates of the effect for every event time (also called time since treatment).
We built upon this code to further
aggregate across event times to produce one overall estimate using the `fit_event_jack_sum()` and `fit_event_jack_sum_hte()` functions.
Here we use `fit_event_jack_sum()` because there is only one adoption cohort.", "blue")`
...
The ATT is equal to the effect estimates by the TWFE model and Callaway and Sant'Anna.

Ideally, we would also show you how to run the Sun and Abraham estimator for 
...
non-dynamic and there are two never-treated states and two ever-treated states.
...
a later example we will add in variability. I will suppress the warnings in this
...
**Takeaway:** When a policy's introduction is staggered in time and the treatment
effect is constant (not dynamic), TWFE can be used to estimate the treatment effect.

`r colorize("Need to also specify that the treatment effects are not heterogeneous by adoption cohort", "blue")`
...
s6_comp1 <- ggplot(s6 %>% filter(state %in% c(1, 2, 3)), aes(x = time, y = outcome)) + 
  geom_line(aes(col = factor(state))) + 
  geom_point(aes(fill = factor(state), pch = ever_trt)) + labs(title = "A) Comparison 1 (Treated vs. never-treated)") +
  geom_vline(aes(xintercept = 4.5), lty = 2) +
  scale_color_manual(values = c("#a6cee3","#1f78b4","#005a32")) +
  scale_fill_manual(values = c("#a6cee3","#1f78b4","#005a32")) +
  theme_bw() + 
...
s6_comp2 <- ggplot(s6 %>% filter(state %in% c(1, 2, 4)), aes(x = time, y = outcome)) + 
  geom_line(aes(col = factor(state))) +
  geom_point(aes(fill = factor(state),  pch = ever_trt)) + labs(title = "B) Comparison 2 (Treated vs. never-treated)") +
  geom_vline(aes(xintercept = 11.5), lty = 2) +
  scale_color_manual(values = c("#a6cee3","#1f78b4","#99000d")) +
  scale_fill_manual(values = c("#a6cee3","#1f78b4","#99000d")) +
  theme_bw()+ 
...
s6_comp4 <- ggplot(s6 %>% filter(state %in% c(3, 4), time >= 5), aes(x = time, y = outcome)) +  
  geom_line(aes(col = factor(state))) + 
  geom_point(aes(fill = factor(state), pch = ever_trt)) + labs(title = "C) Comparison 4 (Later vs. earlier-treated)") +
  geom_vline(aes(xintercept = 11.5), lty = 2) +
  scale_color_manual(values = c("#005a32", "#99000d")) +
  scale_fill_manual(values = c("#005a32", "#99000d")) +
...
s6_comp3 <- ggplot(s6 %>% filter(state %in% c(3, 4), time < 12), aes(x = time, y = outcome)) +   
  geom_line(aes(col = factor(state))) + 
  geom_point(aes(fill = factor(state), pch = ever_trt)) + labs(title = "D) Comparison 3 (Earlier vs. later-treated)") +
  geom_vline(aes(xintercept = 4.5), lty = 2) +
  scale_color_manual(values = c("#005a32", "#99000d")) +
  scale_fill_manual(values = c("#005a32", "#99000d")) +
  scale_shape_manual(values = c(23)) + 
...
`r colorize("I think we will want to include an estimate of the truth here. This depends on what parameter we're interested in, but I think the average dynamic effect is the one that makes the most sense.", "blue")`
...
s6_bacon
```
So we can see the estimates of the treatment effect for each contrast are as we
calculated them earlier. Note that `r round(s6_bacon$weight[3]*100)`% of the weight of the TWFE estimate is on
Contrast 4 -- the one making the improper comparison between an earlier treated
and a later treated state.
...
So we can see that the TWFE regression estimate is biased because it 
incorporates a contrast that we wouldn't want to make in practice between an 
earlier treated and a later treated group. To overcome this, we use the 
Callaway and Sant'Anna estimator. 

`r colorize("Here can we say "one of the other estimators", rather than the "Callaway and Sant'Anna estimator"?.", "blue")`
...
This estimate considered only the clean contrasts (Comparisons 1 and 2 in the figure
above) and combines them by weighting by the total
time in the post-treatment period `r colorize("Is there more to say about how the weighting is done?", "red")`. For contrast 1, there are 16 
time periods post treatment and for contrast 2, there are 9 periods 
post-treatment. Thus this estimate is the weighted average: $10.5\times(16/25) + 7\times(9/25)=$ `r 10.5*(16/25) + 7*(9/25)`

`r colorize("I think this is a good way to explain the weighting. If you want, you could add more detail, for instance: "...combines them into a weighted average, where the weights correspond to each adoption cohort's time spent in the post-treatment period"", "blue")`
...
`r colorize("I think we will also want to acknowledge that the Overall ATT reported here is different, 
and in this case is just the simple average between the two ATTs but in general is a weighted average with the weights proportional to the number of units in each adoption cohort.", "blue")`
...
`r colorize("I think we will also want to acknowledge here that the Overall ATT is the simple average of all effects in the post-treatment time. Here you can see this is (sum(3:18)/length(3:18)) = 10.5", "blue")`
...
`r colorize("This is not working for this example... not sure why...", "red")``
`r colorize("For some reason the max_time_to was being added in seconds rather than days. So, I just increased the max_time_to so it includes all post-treatment times.", "blue")``
...
indicator variables equal the output from the Callaway Sant'Anna when specified using `type = "dynamic"` and the 
`r colorize("and Ben-Michael approach", "blue")`
...
researcher is identifying their parameter of interest -- are you interested in 
estimating a dynamic effect, or does a parameter that summarizes over treatment
time make sense? 

`r colorize("I think these points should be summarized at the end in a recommendation section", "blue")`
I am inclined to start by estimating a dynamic effect to see
if the model supports the presence of a dynamic effect. If so, it is important 
of how the policy operates after being rolled out. If there is no strong support
for a dynamic effect, then I would consider estimating the effects separately by
timing of introduction if that is sensible for the policy change under study 
(i.e., if there are only a few separate time points), or estimating an overall 
summary parameter.

## Scenario 7 (4 states, staggered timing, dynamic and hetergenous treatment effects)

`r colorize("Just a note that we didn't consider this scenario in the simulations.", "blue")`
...
ggplot(s7, aes(x = time, y = outcome)) + 
  geom_point(aes(fill = factor(state), pch = ever_trt)) +
  labs(title = "Scenario 7 (Staggered, dynamic, and heterogeneous)") +
  geom_line(aes(col = factor(state))) + 
...
`r colorize("Again I think here it would be helpful to present a truth estimate, so the reader can see how contrast 4 biases the estimate", "blue")`
...
`r colorize("The DID was negative for scenario 6's contrast 4 too, so I think you can delete the last sentence", "blue")`
...
`r colorize("Here we wrestle with the idea of the truth too. I think we should discuss this earlier and make clear how the investigator will need to define this for their question of interest", "blue")`

The question is, does this estimate come close to what it should be? Well, above
we calculated effect estimates for four separate contrasts, where the fourth
contrast did not satisfy the parallel trends assumption, but the other three
contrasts did. So if I were summarizing across these clean contrasts I would want 
to make a summary estimate across the DID estimates of 18, 5, and 9. One question
is, how much weight do we put on each of the estimates? Another question is, 
do we consider the DID estimate of 9 comparing earlier to later treated states?
It isn't straightforward what the estimate should be when summarizing across 
several contrasts. Let's consider the Goodman Bacon decomposition to see how 
much each comparison contributed to the TWFE estimate: 
...
contributes `r round(s7_bacon$weight[3]*100)`% of the weight to the estimate. We
...
$18\times(16/25) + 5\times(9/25)=$ `r 18*(16/25) + 5*(9/25)` 
...
`r colorize("[DANA: CAN YOU CONFIRM YOU AGREE WITH WHAT I WROTE ABOVE? specifically how the 
simple estimate is calculated.]", "red")`

`r colorize("Yes this is correct.", "blue")`

Alternatively, we can specify `type = "group"` to get separate effect estimates 
according to time of implementation and see that these match our by-hand calculations: 

`r colorize("Again I think we will want to comment on how the Overall ATT is a simple average of the two groups 11.5 = mean(18,5)", "blue")`
...
Note that because the treatment effect is heterogeneous (the later treated state
...
`r colorize("This is not working for this example either, any ideas what it happening?", "red")``
`r colorize("This is the same issue with max_time_to, which I've increased to 10e7 so it should be working now", "blue")`
```{r}
...
Sant'Anna model and `r colorize("and Ben-Michael approach", "blue").
...
case, it is best to use another approach like the Callaway Sant'Anna `r colorize("and Ben-Michael", "blue")` approaches. 

## Scenario 8 (24 states, staggered timing, dynamic and hetergenous treatment effects)

So far, we have considered toy examples where the outcomes didn't incorporate 
noise. This was great for getting started, but doesn't reflect the imprecision
...
```{r, echo=F}
set.seed(112358)
#expand on the s8 dataset to include many states 
state_base1 <- s8_start %>% filter(state == 1)
...
state_base2 <- s8_start %>% filter(state == 2)
...
state_base3 <- s8_start %>% filter(state == 3)
...
state_base4 <- s8_start %>% filter(state == 4)
...
```{r, echo = F, fig.height = 8, fig.width = 10}
s8_comp1 <- ggplot(all_states %>% filter(state <= 35), aes(x = time, y = outcome)) +   
...
s8_comp2 <- ggplot(all_states %>% filter(state <= 25 | state >= 40), aes(x = time, y = outcome)) +   
  geom_line(aes(col = factor(ever_trt), group = state)) + 
...
s8_comp4 <- ggplot(all_states %>% filter(state >= 30, time >= 15), aes(x = time, y = outcome)) +
  geom_line(aes(col = factor(time_first_treat), group = state)) + 
...
s8_comp3 <- ggplot(all_states %>% filter(state >= 30, time < 22), aes(x = time, y = outcome)) +   
  geom_line(aes(col = factor(time_first_treat), group = state)) + 
  geom_point(aes(fill = factor(time_first_treat), group = state, pch = ever_trt), show.legend = F) +
...
we want to estimate confidence intervals and to to that correctly, we need to 
...
TWFE estimate. I set up this scenario such that the effects should be the same
as for scenario 7, though this time the pre-treatment period is longer than in 
scenario 7.

`r colorize("I don't think you need to bring in scenario 7, it made me think I needed to go remind myself what it was, but you have the relevant info below so I think it's distracting. ", "blue")`
...
`r colorize("again I think the control group should be "nevertreated". I don't think it affects anything because the never treated are included as controls, but I'm not sure you necessary want to include the not yet treated as controls for this example., "blue")`
```{r}
...
```
The CS estimate is `r s8_cs_ag$overall.att %>% round(2)` with 95% CI `r lb %>% round(2)` 
to `r ub %>% round(2)`.

The simple ATT is a weighted average (based on the amount of time in the 
post-treatment period) of the estimated effects for the treated groups. To figure
out what the group specific estimate are we can ask for the estimates by group:
...
`r colorize("NOTE: (Was expecting 18 and 5 as the group specific estimates -- possible we are off just due to random error - need to rerun with a different seed or with less variability and see if that is true)", "red")`

`r colorize("Yeah, I think it might be a random error issue. The confidence intervals cover 5 so I am not too worried.", "blue")`

Finally, we can look at these effect estimated separately for each time since 
...
We can also calculate the Sun & Abraham estimator. We couldn't run this for the
other scenarios because the function throws an error when if cannot estimate the
standard errors for those scenarios. To use the `staggered_sa` function, we 
need to add a new variable where the `time_first_treat` variable is set to `Inf` 
for all states that were never-treated:

`r colorize("Typo, would replace the previous paragraph with: 

Now we can also calculate effects using the Sun & Abraham estimator. We couldn't run this estimator for the
other scenarios because the function throws an error if it's unable to estimate
standard errors. To use the `staggered_sa` function, we 
need to add a new variable where the `time_first_treat` variable is set to `Inf` 
for all states that were never-treated:", "blue")`
...
use the newly-defined `time_first_treat2` as the grouping variable `g`. 
...
The estimate of the treatment effect is `r s8_sa$estimate` and we can calculate
its confidence interval using either of these:
...
For this example, the Callaway & Sant'Anna and Sun & Abraham estimates are quite
close and have overlapping confidence intervals. They're both higher than the 
TWFE estimate because they do not use information based on the forbidden contrast.

### Target Trial Estimator

We can also consider the Target trial estimator.

Their function relies on the date variable being a date object rather than a
...
## Scenario 9 (same as previous but with less pre-exposure time)

In this scenario, we use the same data from Scenario 8, except we limit the 
pre-intervention point to 5 time units, rather than the 15 
in Scenario 8. The amount of pre-intervention time should not affect the size of 
...
to `r round(ub2, 1)`. Let's compare this with the estimate from Scenario 8:

|           | Estimate (95% CI) | 
|----------:|:------------------|
|Scenario 8 | `r paste0(round(est, 1), " (", sprintf("%.1f", round(lb, 1)), ",", round(ub, 1), ")")`  | 
|Scenario 9 | `r paste0(round(est2, 1), " (", sprintf("%.1f", round(lb2, 1)), ",", round(ub2, 1), ")")` |

The estimate for Scenario 9 is much lower than that for Scenario
8. 

### Comparison of Goodman Bacon decomposition weights

Let's see how the weights from the Bacon decomposition for Scenario 9 compare
to Scenario 8. 
...
```{r, echo=F}
s8_weights <- s8_bacon %>% select(weight) %>% rename(`Scenario 8 weight` = weight)  
compare_bacon <- s9_bacon %>% rename(`Scenario 9 weight` = weight) 
comparison <- bind_cols(s8_weights, compare_bacon) %>% relocate(`Scenario 8 weight`, .before = `Scenario 9 weight`)
comparison %>% mutate(`Weight difference` = `Scenario 9 weight` - `Scenario 8 weight`) %>% 
  mutate_if(is.numeric, ~round(., 2)) %>% relocate(type, .after = last_col())
```

More weight is put on the states treated at time=22 in Scenario
9 vs. 8 and less weight on the states treated at time=15. This is because the 
...
the panel. For Scenario 8, the states with treatment changes at time 15 are exactly
in the middle of the panel (which had 30 time units), but for Scenario 9, with 
...
What about Sun Abraham's estimate?
...
round(c(s8_sa$estimate, s8_sa$estimate - 1.96*s8_sa$se_neyman, s8_sa$estimate + 1.96*s8_sa$se_neyman), 2)
```
The Sun Abraham estimate also stays exactly the same!

### Comparison of the Target trial estimates

`r colorize("Is the target trial the same too? Also, is there a reason you're using the HTE version here but not previously?", "blue")`
...
**Takeway:** The TWFE estimate is affected by the length of the panel. It places
more weight on contrasts that are closer to the middle of the panel. The 
Callaway Sant'Anna and Sun Abraham estimates do not have this issue.

`r colorize("Add target trial to this takeaway too.", "blue")`
