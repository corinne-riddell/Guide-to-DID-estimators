...
`r colorize("TODO1: check with Dana that I am using the correct fit_event_jack functions across the different scenarios", "red")`
...
1. Goin DE, Riddell CA (2022). Comparing two-way fixed effects and new estimators for difference-in-differences: A simulation study and empirical example. Pre-print. ADD LINK.

2. Riddell CA, Goin DE (2022). Guide for comparing estimators of policy change effects on health. Pre-print. ADD LINK.
...
the TWFE approach is biased. In the latter case, we show alternative methods to
estimation to overcome the issue.
...
found in Ben-Michael's linked GitHub repository. The other helper functions are ones
...
each time post-policy change. Take the average across all treated states, with one average
calculated for each time point. The causal effect of the policy at the time of 
first introduction is 3. This was calculated by noting that the outcome increased
by 3 units between successive periods pre-policy change in the treated states
and control states. At the time the policy was introduced the outcome increased by 
6 in the treated states vs. 3 in control states, for a policy effect of 6-3=3. 
In the following period (time since treatment = 2), the causal effect of the policy 
change was 4, by a calculation of the same form. As time goes on, the effect of 
the policy change increases by one in each successive period:
...
This is the case for both the earlier- and the later- treated state. So, when 
estimating the effects dynamically, these are the time-since-treatment-specific
parameters we aim to estimate.

2. **The average dynamic effect**: You may prefer a summary estimate, that is 
aggregated across all post-treatment time into one number. In this case, we can 
take the average of the causal effects across all the time points:

$\frac{3+4+...+18}{\text{number of post-treatment times}} = 136/16 = 8.5$

`r colorize("I'm getting 8.5 not 10.5, different from Callaway Sant'Anna Overall ATT for the dynamic specification... any ideas what it off? See my calculation here:", "red")`

```{r}
sum(seq(3:18))/length(3:18)
```
...
`r colorize("I added the recommendation below. Do you like it here?", "red")`
...
When the causal effect is staggered, dynamic, and heterogeneous, there are four
main ways to summarize the effect: 

i) Separately by adoption cohort and post-treatment time point
ii) One ATT for each adoption cohort summarized over time: If you only consider
the contrasts compared to never treated units, then the ATT estimates are 10.5 
and 7 for states that introduced the policy change at time 5 and 12, respectively.
iii) One ATT for each time point summarized across adoption cohorts
iv) One over-arching ATT (A single point estimate)
...
`r colorize("I'm tryign to incorporate this change but running into issues because I don't think we need to pick one truth and say that it is THE truth -- think it is better for the reader to understand there are several ways to summarize and so several truths. The other thing that is hard is that the bacon decomp and callaway and santanna sections flow nicely after presenting the 4 contrasts so just need to keep that flow in mind when writing this section up.", "red")`
...
`r colorize("Dana can you read over about the fit_ functions below and see what you think?", "red")`
...
specifying `type = "dynamic"` in the `aggte()` function.
...
Here the Overall ATT is a simple average of the group-specific ATTs (`r round((s8_cs_ag2$att.egt[1] + s8_cs_ag2$att.egt[2])/2, 4)`)
