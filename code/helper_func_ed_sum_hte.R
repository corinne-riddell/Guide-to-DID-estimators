...
fit_event_manual_sum_hte <- function(outcome_var, date_var,
  ...
  cohort_weights <- tmpdat %>% 
    group_by(policy_var) %>%
    count() %>%
    ungroup() %>% filter(policy_var %in% policy_times) %>% 
    mutate(pct = n / sum(n)) %>%
    ungroup() %>%
  ...
  map_df(policy_times,
         function(x) {
           ...
           x_dat %>%
             ...
             select(-date_var) %>% 
             group_by(cohort) %>%
             summarise(estimate = mean(estimate[event_time>=0])) %>% 
             ungroup() 
         }) -> cohort_dids
  
  cohort_dids %>% 
    left_join(cohort_weights, by = "cohort") %>%
    summarise(estimate = sum(estimate * pct)) %>%
    ungroup()
  
}

fit_event_jack_sum_hte <- function(outcome_var, date_var, unit_var,
                               policy_var, data, max_time_to = NULL) {
  ...
  map_df(units,
         function(i) {
           ...
           fit_event_manual_sum_hte(outcome_var, date_var,
                                policy_var, data[data[[unit_var]] != i,],
                                max_time_to) %>%
             mutate(dropped = i)
         }) -> jack_ests
  # compute standard errors and join in with results
  full_res <- fit_event_manual_sum_hte(outcome_var, date_var, policy_var,
                                   data, max_time_to)
  jack_ests %>%
